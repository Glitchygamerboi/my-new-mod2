...
public class biome {
    private static final ConfiguredSurfaceBuilder<TernarySurfaceConfig> BOREAL_SURFACE_BUILDER = SurfaceBuilder.GIANT_TREE_TAIGA.withConfig(new TernarySurfaceConfig(BlockRegistry.BOREAL_DIRT.getDefaultState(), BlockRegistry.BOREAL_DIRT.getDefaultState(), BlockRegistry.NEPHELINE.getDefaultState()));private static final Biome BOREAL = createBoreal();private static Biome createBoreal() {// We specify what entities spawn and what features generate in the biome// Aside from some structures, trees, rocks, plants and//   custom entities, these are mostly the same for each biome.// Vanilla configured features for biomes are defined in DefaultBiomeFeatures
